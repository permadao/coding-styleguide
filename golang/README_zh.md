...
- 标准函数：在 demo.go 中原则上只实现三个方法 New(), Run(), Close()

- 公用和私有函数：在 **.go 文件中，只有需要被其他文件调用的方法首字符才大写。

- 注释：开发过程中代码功能未完成使用大写 `todo:` ，特殊说明使用 `notice:`

- Schema 命名规范：如果分技术列别，技术类型在前，业务名称在后。以 http 请求和响应的 struct 为例子：
    ...
    good:
      ...
      ResponseInfo
    ```
- 顺序：文件中 public 方法统一写在 private 方法之前
...
- 参数：相同属性的传参写到一起，返回值如此
   ```go
   AA(ethRpc, moonRpc, cfxRpc string, db *Db) {}
   ```

- 错误：存在 error 返回值，error 写在返回值最后一位
   ```go
   AA(ethRpc, moonRpc, cfxRpc string, db *Db) (string,int,error) {}
   ```

- 指针的使用：函数参数避免指针类型。如果是 slice, map 传参之前可以 copy 一份

- 判断：避免使用多重 if- else if -else, 请替换成 switch

- 闭包：不要使用闭包，因为你没法控制闭包中调用的外部变量是如何变化
    ``` 
    bad:
        for i:=0; i <10; i ++ {
            go func() {
...
- 锁：使用 locker.Lock() 之后必须立即声明 defer locker.Unlocker()
  ```go
  bad: 
  func AA() {
  	locker.Lock()
...
- inti()：尽量避免使用 init() 函数，因为加载 init() 函数中路径不可控，并且 init() 中逻辑不显式调用执行。请把 init() 中的处理放在 demo.go/New() 或者 Run() 中。

- slice：使用 slice 或者 map，请在 make 的时候指定 cap, 这样 append 的时候会更高效以及节省代码运行时开辟的内存空间。
  ```go
  tokenList := make([]severSchema.TokenInfo, 0, 50)
  ```

- chan：正常情况下不会使用到缓冲 channel, 如果必须要使用缓冲 channel, 请备注好为什么 cap 设置为该值。

- 同名 package：需要重命名 import package 的时候需要遵循就近原则，本模块的不需要重命名，名字冲突的其他模块命名需要加上模块名。
    ```go
    import (
...
- http api：api 返回结果必须使用 struct 形式
  ```go
  bad:
